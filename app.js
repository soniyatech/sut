...
io.on("connect", function(socket) {
  ...
  socket.on("broswer processing reading", function(data) {
    console.log(
      "socket.io - broswer processing on new reading: %s",
      JSON.stringify(data)
    );
  });
});
...
var UDP_server = function(UDP_listening_port) {
  ...
  udp_socket.on("listening", function() {
    ...
    console.log(
      "UDP_server - UDP socket listening on %s:%s",
      address.address,
      address.port
    );
  });
  ...
  udp_socket.on("message", function(msg, rinfo) {
    console.log(
      "UDP_server - incoming UDP from %s:%s: %s",
      rinfo.address,
      rinfo.port,
      msg
    );
...
var reversegeo = function(reading) {
  ...
  if (reading.ACQUSITION) {
    var latitude = reading.LATITUDE,
      longitude = reading.LONGITUDE;
    var rev_geo_uri = format(
      "http://%s:%d/soniyatech/GisServlet?action=rgeocode&lat=%d&lon=%d",
      geo_server_ip,
      geo_server_port,
      latitude,
      longitude
...
var Punttoo = function() {
  ...
  appEmitter.on("incoming_UDP_is_Punttoo", function(signal) {
    ...
    if (signal.indexOf("ACK") >= 0) {
      ...
      console.log(
        "Punttoo - incoming_UDP_Punttoo_ack_parsed: %s,%s,%s,%s,%s",
        ack.MODEL,
        ack.IMEI,
        ack.ACK,
        ack.ITEM,
      ...
      intermediate =
        "Punttoo,012813006342879,$C,7,WCDMA,310,410,0xDE76,5872298,$P,2,70,$GPRMC,053436.000,A,3737.9322,N,12194.1046,W,1.01,186.75,011116,,,A*72";
      var c_csq,
        c_mode,
        c_country,
        c_operator,
      ...
      if (GPRMC >= 0) {
        ...
        if (A >= 0 && V < 0) {
          ...
          if (W >= 0) {
            intermediate = signal.slice(W + 3);
          }
          if (E >= 0) {
            intermediate = signal.slice(E + 3);
          }
          knot = intermediate.slice(0, intermediate.indexOf(","));
          if (distance_unit.includes("metric")) {
            knot = 1.15 * knot;
          }
      ...
      console.log(
        "Punttoo - incoming_UDP_Punttoo_reading_parsed: %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s",
        reading.MODEL,
        reading.IMEI,
        reading.C_CSQ,
        reading.C_MODE,
...
var mongo_uri =
  "mongodb+srv://united:j3byrcc@cluster0-ng5j6.azure.mongodb.net/united?retryWrites=true";
...
module.exports = { app: app, http_server: http_server, io: io };
